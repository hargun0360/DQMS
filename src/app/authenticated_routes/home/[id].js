...
const StoreInfo = () => {
  ...
  const handleJoinQueue = (storeId, userId) => {
    socket.emit("joinQueue", { storeId, userId });

    socket.on("queueUpdated", (data) => {
      console.log(data);
      Alert.alert(
        "Queue Update",
        `You are assigned to counter ${data.counterAlloted}. Estimated wait time: ${data.waitingTime} minutes.`,
        [
          {
            text: "OK",
            onPress: () => {
              router.push("/authenticated_routes/standing");
            },
          },
        ]
      );
        ...
        onPress: () => {
          ...
          RazorpayCheckout.open(options)
            .then((data) => {
              ...
              handleJoinQueue("662d689dd2dadc6865b709c9 ", "110013");
            })
            .catch((error) => {
              // handle failure
              console.log(error);
